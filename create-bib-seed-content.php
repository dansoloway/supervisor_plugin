<?php
/**
 * Create Seed Content for Bibliography Items
 * Creates 2-3 qa_bib_items per qa_tags category
 */
...
echo '<h1 style="color: #333;">יצירת תוכן זרע לביבליוגרפיה</h1>';
...
if (empty($tags)) {
    echo '<p style="color: red;">לא נמצאו קטגוריות qa_tags</p>';
    echo '</div>';
    exit;
}
...
foreach ($tags as $tag) {
    if (!isset($seed_content[$tag->name])) {
        echo '<p style="color: orange;">לא נמצא תוכן זרע לקטגוריה: ' . esc_html($tag->name) . '</p>';
        continue;
    }
    
    echo '<h3 style="color: #0073aa; margin-top: 20px;">קטגוריה: ' . esc_html($tag->name) . '</h3>';
    ...
    foreach ($seed_content[$tag->name] as $item) {
        ...
        echo '<p style="color: green;">✓ נוצר פריט: ' . esc_html($item['title']) . '</p>';
        $created_count++;
    }
}

echo '<h2 style="color: #333; margin-top: 30px;">סיכום</h2>';
echo '<p style="font-size: 18px; font-weight: bold; color: #0073aa;">נוצרו ' . $created_count . ' פריטי ביבליוגרפיה</p>';

if (!empty($errors)) {
    echo '<h3 style="color: red;">שגיאות:</h3>';
